...
const validateCred = (arr) => {
  ...
  const arrReverse = arrCopy.reverse();
  const newNumbers = [];
  for (let i = 0; i < arrReverse.length; i++) {
    if (i % 2 === 1) {
      var biggerNumber = arrReverse[i] * 2;
      if (biggerNumber > 9) {
        biggerNumber = biggerNumber - 9;
        newNumbers.push(biggerNumber);
      } else {
        newNumbers.push(biggerNumber);
      }
    } else {
      newNumbers.push(arrReverse[i]);
    }
  }

  var sum = newNumbers.reduce(
